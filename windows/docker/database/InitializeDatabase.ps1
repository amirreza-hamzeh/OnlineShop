...
$mdfPath = 'c:\database\OnlineShopDB_Primary.mdf'
if ((Test-Path $mdfPath) -eq $true) {
    $sqlcmd = "CREATE DATABASE AssetsDB ON (FILENAME = N'$mdfPath')"
    $ldfPath = 'c:\database\OnlineShopDB_Primary.ldf'
    if ((Test-Path $mdfPath) -eq $true) {
...
& $SqlPackagePath  `
	/sf:Docker.OnlineShop.Database.dacpac `
	/a:Script /op:deploy.sql /p:CommentOutSetVarDeclarations=true `
	/tsn:.\SQLEXPRESS /tdn:OnlineShopDB /tu:sa /tp:$sa_password

# deploy the create or upgrade script
$SqlCmdVars = "DatabaseName=OnlineShopDB", "DefaultFilePrefix=OnlineShopDB", "DefaultDataPath=c:\database\", "DefaultLogPath=c:\database\"
